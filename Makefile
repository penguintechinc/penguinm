...
# melos runs from a global activation (`dart pub global run`) — `dart run melos`
# re-resolves the pub workspace via `dart pub`, which cannot resolve the
# Flutter-SDK deps (flutter_localizations, pulled in by Gazer). ensure-melos
# activates the pinned version idempotently; melos targets depend on it.
MELOS := dart pub global run melos

.PHONY: ensure-melos
ensure-melos:
	@dart pub global activate melos 8.7.0 >/dev/null

setup: install-hooks ensure-melos ## Verify Flutter 3.44.8, dart pub get, melos bootstrap, install git hooks
...
bootstrap: ensure-melos ## Resolve workspace dependencies via melos bootstrap
	$(MELOS) bootstrap

lint: ensure-melos ## dart format check + flutter analyze (zero infos/warnings) across the workspace
...
test: ensure-melos ## Run every package's test suite with --coverage (coverage/lcov.info per package)
	$(MELOS) exec -c 4 --dir-exists=test -- flutter test --coverage --timeout=60s

test-unit: ensure-melos ## Run unit tests only (test/, excludes integration_test/)
	$(MELOS) exec -c 4 --dir-exists=test -- flutter test --coverage --timeout=60s test/

test-integration: ensure-melos ## Run integration_test/ suites (requires a running Android emulator, API 35 x86_64)
	@echo "test-integration requires a connected Android emulator (API 35, x86_64); see docs/TESTING.md"
	$(MELOS) exec -c 4 --dir-exists=integration_test -- flutter test --timeout=60s integration_test/
...
smoke-test: ensure-melos ## bootstrap + analyze + penguin_reference tests + telemetry-validate + check-pins + check-logging (<2 min)
	$(MELOS) bootstrap
	$(MELOS) exec -- flutter analyze --fatal-infos
...
seed-mock-data: ensure-melos ## Regenerate test/fixtures from penguin_testing generators (prints file counts)
	$(MELOS) exec --dir-exists=test -- dart run penguin_testing:seed_fixtures
