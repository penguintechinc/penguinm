...
void main() {
  test(
    'pipelineStateProvider emits the controller current state, then follows its stream',
    () async {
  ...
  test(
    'streamStatsProvider emits the zero snapshot, then follows the controller\'s aggregated stats',
    () async {
      final host = FakeGazerHostApi();
      final bridge = NativeEventBridge();
      final controller = PipelineController(
