...
AppManifest _buildTestManifest(AppConfig config) {
  return AppManifest(
    ...
    auth: AuthConfig.hosted(
      ...
      clientId: '{{org}}.{{name}}',
      redirectUri: '{{org}}.{{name}}.dev://oauth/callback',
    ),
    features: const [],
  );
}
...
void main() {
  group('Telemetry smoke test', () {
    ...
    // indefinitely with no output. `test()` runs in the real zone instead,
    // so every Timer here — the 5s network timeouts and the 100ms delay —
    // elapses in real wall-clock time.
